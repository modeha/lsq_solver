...
%-----------------------------------------------------------------------
  ...
  z0min     = options.z0min;
  mu0       = options.mu0;
  Method    = options.Method;     % 1=Cholesky  2=QR  3=LSMR  4=MINRES  21=SQD
  ...
  if PriLev > 0
    ...
    fprintf('\n\nMethod   = %8g     (1=chol  2=QR  3=LSMR  4=MINRES  21=SQD)\n', Method)
  ...
  if operator
    if Method==3 || Method ==4
      ...
      fprintf('\n\n When A is an operator, we have to use Method = 3 or Method = 4')
      Method = 3;
    end
  end
...
%       dx        =   H.*(grad - w);
 ...
 %  stepx  = min( [stepx1,   stepx2,   stepxL,   stepxU] );
    ...
    elseif step   <= 1e-10
      ...
      % Reduce atol for LSMR (and SYMMLQ).
...
%              global variables. 
% 3  Apr 2012: Renamed to pdxxxlsmrmat.
