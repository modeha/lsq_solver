function [iter,primobj,gap,x] = l1_ls_direct(A,varargin)
%
...
for ntiter = 0:MAX_NT_ITER
    ...
    %------------------------------------------------------------
    %Uses a Cholesky decomposition instead of the conjugate gradient
    %------------------------------------------------------------
    %[dxu,pflg] = pcg(@(dxu)AXfunc_l1_ls(dxu,A,At,d1,d2),-gradphi);
    [hessphi] = AXfunc_l1_ls(x,A,At,d1,d2);
    %[dxu,pflg] = pcg(hessphi,-gradphi,[],200);
    dxu = hessphi\-gradphi;
    %dxu = sqd_solve(hessphi,-gradphi);
...
%       COMPUTE AX (PCG)
%------------------------------------------------------------
function [hessphi] = AXfunc_l1_ls(x,A,At,d1,d2)
%
% y = hessphi*[x1;x2],
%
 hessphi = [A'*A*2+diag(d1) , diag(d2); diag(d2) , diag(d1)];
