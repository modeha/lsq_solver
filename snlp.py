...
class SlackNLP( NLPModel ):
    ...
    def __init__(self, nlp, keep_variable_bounds=False, **kwargs):
        ...
        # Create more lists
        bot = self.original_n; self.sLL = List(range(bot, bot + nlp.nlowerC))
        bot += nlp.nlowerC;    self.sLR = List(range(bot, bot + nlp.nrangeC))
        bot += nlp.nrangeC;    self.sUU = List(range(bot, bot + nlp.nupperC))
        bot += nlp.nupperC;    self.sUR = List(range(bot, bot + nlp.nrangeC))
        ...
        if keep_variable_bounds==False:
            ...
            # Number of slacks for the bounds
            nSlacks_t = n_var_low + n_var_up; self.nSlacks_t = nSlacks_t

            # Create more lists
            bot = self.original_n + nSlacks_s; self.tLL = List(range(bot, bot + nlp.nlowerB))
            bot += nlp.nlowerB;                self.tLR = List(range(bot, bot + nlp.nrangeB))
            bot += nlp.nrangeB;                self.tUU = List(range(bot, bot + nlp.nupperB))
            bot += nlp.nupperB;                self.tUR = List(range(bot, bot + nlp.nrangeB))
