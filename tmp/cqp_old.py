...
class RegLSQInteriorPointSolver4x4(RegQPInteriorPointSolver3x3):
    ...
    def solve(self, **kwargs):
        ...
        """
        qp = self.lsq
        #mu = -1 ; alpha_p = -1 ; alpha_d = -1 ; nres =  -1 ; regdu = -1 
        #rho_q = -1 ;  del_w = -1 ; mins = -1 ; minz = -1 ; maxs =  -1                 
        (n, nx, nr, ns, m) = self.get_dimensions()
        ...
        while not finished:
            ...
            if kktResid <= tolerance and iter > 0 :
                status = 'Optimal solution found'
                short_status = 'opt'
                finished = True
                continue
        ...
        self.z  = z
        self.iter = iter
        self.pResid = pResid ; self.cResid = cResid ; self.dResid = dResid; self.qNorm= qNorm ; self.wNorm = wNorm
        self.mu = mu ; self.regdu = regdu 
        self.alpha_p = alpha_p ; self.alpha_d = alpha_d ; self.nres =  nres ; self.rho_q = rho_q 
        self.del_w = del_w ; self.mins = mins ; self.minz = minz ; self.maxs =  maxs 
...
class RegLSQInteriorPointIterativeSolver4x4(RegLSQInteriorPointSolver4x4):    
    ...
    def solveSystem(self, rhs, itref_threshold=1.0e-5, nitrefmax=5):
        ...
        if preconditioner:
            ...
            lsqr.solve(rhs,atol = tol_, btol = tol,show=False)
            sol_final=lsqr.x
