...
def partial_DCT(n = 10, m = 4, delta = 1.0e-05):
    ...
    #print np.eye(new_Q.shape[1])[:,0]
    #print new_Q*np.eye(new_Q.shape[1])[:,0]
    p, n = new_Q.shape
    m, n = B.shape
...
if __name__ == "__main__":
    ...
    from pykrylov.linop import LinearOperator
    from pykrylov.linop import *
    partial_DCT(n = 16, m = 8, delta = 1.0e-05)
    n=4;m=50000;
    print sprandvec(m,n)
    xx

    J = np.array(range(max(m,n)))
    A = LinearOperator(nargin=max(m,n), nargout=min(m,n), matvec=lambda v: dct(v),
                           matvec_transp=lambda v: idct(z_v(max(m,n),J,v)))
    #print "-"*80
    ...
    #print dct(v)[0,:],'dct'
    #print 
    #b=A.T*np.eye(A.shape[0])[0,:]
    #print b
    #print idct(z_v(n,J,v)),'idct'
